...
function sign() {
    ...
    # remove prefix from openssl, e.g. SHA256(stdin)= 
    s="${s#*= }"
...
l_header="$(base64URLEncode ${l_header})" || exit 1
l_body="$(base64URLEncode ${l_body})" || exit 1
l_signing_string=${l_header}.${l_body}
l_signature=$(sign ${l_signing_string}) || exit 1
...
echo ""
echo "signing digest: ${l_signing_digest}"
echo ""


# HTTP inputs
l_host_port="localhost:8443"
l_req_url=/ai-transform-groq/admin/reload
